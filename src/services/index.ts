...
export const getHeroProfile = async (
    heroId: string
): Promise<Types.IHeroProfile> => {
    try {
        const result = await fetcher<Types.IHeroProfile>(
            `${API_DOMAIN}/heroes/${heroId}/profile`,
            {
                headers: {
                    Accept: "application/json",
                },
